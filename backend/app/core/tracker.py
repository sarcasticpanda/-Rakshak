...
# Size constraints (must match detector config)
MAX_BOX_WIDTH = 350
MAX_BOX_HEIGHT = 600
MAX_BOX_AREA = 100000
MIN_BOX_AREA = 800
...
class Track:
    ...
    def is_valid_size(self, frame_w: int, frame_h: int) -> bool:
        """Check if bbox is valid size (not too big)"""
        ...
        if w <= 0 or h <= 0:
            return False
        if w > MAX_BOX_WIDTH or w > frame_w * 0.25:
            return False
        if h > MAX_BOX_HEIGHT or h > frame_h * 0.5:
            return False
...
class ByteTracker:
    ...
    def __init__(
        ...
        track_buffer: int = 15,      # Reduced from 30 - faster cleanup
        match_thresh: float = 0.4,   # Stricter matching
        min_hits: int = 3,           # Need 3 hits to confirm
        max_age: int = 10            # Reduced from 30 - remove ghosts faster
    ):
    ...
    def update(self, detections: List[dict], frame_size: Tuple[int, int] = None) -> List[dict]:
        ...
        valid_tracks = [t for t in self.tracks if t.is_valid_size(*self.frame_size)]
    ...
    def _cleanup_tracks(self):
        ...
        for track in self.tracks:
            ...
            if not track.is_valid_size(*self.frame_size):
                continue
            
            # Remove if bbox went out of frame
            if track.bbox[0] < -100 or track.bbox[1] < -100:
                continue
            if track.bbox[2] > self.frame_size[0] + 100:
                continue
            if track.bbox[3] > self.frame_size[1] + 100:
                continue
    ...
    def _get_output(self) -> List[dict]:
        ...
        for track in self.tracks:
            ...
            if track.hits < self.min_hits:
                continue
            if track.time_since_update > 3:  # Only show recently updated tracks
                continue
            if not track.is_valid_size(*self.frame_size):
                continue
